...
public class ProdutoService
{
    private readonly IProdutoReader _produtoReader;
    private readonly IProdutoWriter _produtoWriter;
    private readonly ICategoriaReader _categoriaReader;
    private readonly IProdutoLojaReader _produtoLojaReader;

    public ProdutoService(IProdutoReader produtoReader,
                          IProdutoWriter produtoWriter,
                          ICategoriaReader categoriaReader,
                          IProdutoLojaReader produtoLojaReader)
    {
        _produtoReader = produtoReader;
        _produtoWriter = produtoWriter;
    ...
    public async Task<IEnumerable<Produto>> GetAllProdutos()
    {
        return await _produtoReader.GetAllProdutos();
    }

    public async Task<ProdutoViewModel?> GetProdutoDetalhesViewModel(int id)
    {
        var produto = await _produtoReader.GetProdutoById(id);
        if (produto == null) return null;

        var produtoLoja = await _produtoLojaReader.GetProdutoLojaByProduto(id);
        var categorias = await _categoriaReader.GetAllCategorias();
    ...
    public async Task CriarProdutoAsync(ProdutoViewModel model)
    {
        ...
        await _produtoWriter.AddProduto(produto);
    }
}