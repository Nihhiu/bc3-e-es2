...
namespace ComparacaoPrecos.Repository
{
    public class ProdutoRepository : IProdutoReader, IProdutoWriter
    {
        private readonly ApplicationDbContext _context;
        ...
        public async Task<Produto> GetProdutoById(int id) 
        {
            var produto = await _context.Produto.FirstOrDefaultAsync(p => p.ProdutoID == id && !p.Deleted) 
                ?? throw new KeyNotFoundException($"Produto with ID {id} not found or is deleted.");
            return produto;
        }
